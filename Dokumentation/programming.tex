...
The \textit{setUp()} method creates fixtures, which will be stored in the database, before each test case will be run. This ensures that all "retrieving"-methods have something to read and all "remove"-methods have something to delete.
...
which will be executed by the use of an \textit{DBConnection} object, which contains an actual database connection.
...
The sole purpose of the \textit{DBConnection} class is to return a open database connection or the caller. This is achieved by loading the configuration file and read out the settings, so the \textit{DBConnection} class can decide which kind of database connection it will return the the calling method as response.
