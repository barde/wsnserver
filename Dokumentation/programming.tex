...
The sum of the requirements elevated the Model-View-Controller design pattern.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{pic/Concept.png}%
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{pic/UserReadAll.png}%
...
It is advisable to use a MySQL database if more than one wireless sensor node is connected to the \textit{wsnserver}, because the SQLite database uses a file to store the information, which allows only one write access at the time. If more than one device or applications are writing to the SQLite database at the same time, the database will crash and errors will occur.
